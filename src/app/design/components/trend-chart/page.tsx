...
const monthly: TrendDataset = {
  ...
  series: [
    { key: "spend", label: "Spend", color: "var(--color-brand)", values: [480, 520, 450, 560, 600, 540, 620, 680, 590, 750, 710, 850] },
  ],
};
...
const weekly: TrendDataset = {
  ...
  series: [
    { key: "spend", label: "Spend", color: "var(--color-brand)", values: [110, 125, 118, 140, 135, 150, 145, 160, 155, 170, 165, 180, 175, 190, 185, 200] },
  ],
};
...
const daily: TrendDataset = {
  ...
  series: [
    { key: "spend", label: "Spend", color: "var(--color-brand)", values: Array.from({ length: 30 }, (_, i) => 18 + Math.round(5 * Math.sin(i / 1.8))) },
  ],
};
...
export default function TrendChartPage() {
  const [chartStyle, setChartStyle] = useState<(typeof chartStyles)[number]>("default");
  ...
  return (
    <div className={styles.page}>
      ...
      <p className={styles.subtitle}>
        ...
        — swaps the entire x-axis to that granularity's own dataset, and its
        label format: months for Monthly, "date month" for Weekly/Daily)
        plus secondary prev/next pan arrows that page through the underlying
        data, minimal or default (axes/gridlines + hatch-filled area) style.
        The line animates in on load and on every pan/granularity change.
      ...
      <div className={styles.section}>
        ...
        <div className={styles.row}>
          <StateToggle options={chartStyles} value={chartStyle} onChange={setChartStyle} />
        </div>
      </div>

      <div className={styles.section}>
        <div className={styles.sectionTitle}>Advertiser Overview — Spend-over-time chart</div>
        <TrendChart
