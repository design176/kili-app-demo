...
- Next.js 16 App Router, TypeScript, no backend — `src/app/*` are the real product screens (`overview`, `campaigns` + `[id]` + `new`, `billing`, `settings`, `earnings`, `integration`, `pixel-tracking`, `login`); `src/app/design/*` is the component catalog/demo area.
